...
const MapControls = ((props:MapControlProps) => {
	...
	const changePredictionDistance = (event:React.ChangeEvent<HTMLInputElement>) => {
	...
	return (
	  	<div className="absolute bottom-8 right-8 w-44 h-48 z-6000 border-4 border-black bg-black bg-opacity-50 pt-1">
			...
			<label className="w-full h-8 text-white">Ennuste (h): {predictionDistance}</label>
			<br/>
			<input type="range" min={1} max={3}  className="w-full h-8 " id="predictionDistance" step={1}
			onChange={changePredictionDistance} value={predictionDistance}></input>

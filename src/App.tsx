  ...
  return (
    <div className="App">
      ...
      <div>
        ...
        <div
          className={`absolute w-full h-full top-0 bottom-0 bg-black	z-6000 transition-all duration-500 ${
            showLoadView ? "opacity-50 visible" : "opacity-0 invisible"
          }`}
        ></div>

        <Loop
          className={`loadingSpinner top-[50%] left-[50%] absolute z-6000 transition-all duration-500 ${
            showLoadView ? "opacity-100 visible" : "opacity-0 visible"
          }`}
        />

        <div
          className={
            `h-[95vh] w-[98%] mt-[2vh] ml-[1%] border rounded-3xl` /* h-85*/
          }
        >
          <MapComponent
            mapRef={mapRef}
            ref={mapContainerRef}
            loadingDone={() => hideLoadView()}
          />
        </div>
      </div>
      ...
      {
        <button
          className={`absolute w-14 h-14 top-8 right-8 z-6000 text-white bg-black opacity-30 p-1 rounded-md hover:opacity-50`}
      ...
      <div
        className={`absolute w-24 h-10 top-8 right-16 z-6000
                            ${showDropDown ? "visible" : "invisible"}`}
      >
        <CloseHandler
          active={showDropDown && !showAbout}
